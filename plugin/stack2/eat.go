...
// 吃猫猫
func (d *data) eat(handler *msg.Handler, r ...replacer) message.ID {
	if len(r) == 0 {
		r = []replacer{l10n(cat)}
	}
	var (
	...
	if m.getTypeID(handler) < 小老虎 {
		...
		return sendWithImageFail(handler, r[0], `老虎以上才可以吃猫猫——`)
	}
	// 未在叠猫猫的队列
	dn = d.getNoStack()
	// 执行吃猫猫
	if !d.doEat(handler, &m, r...) {
...
// 执行吃猫猫
func (d *data) doEat(handler *msg.Handler, m *meow, r ...replacer) bool {
	if len(r) == 0 {
		r = []replacer{l10n(cat)}
	}
	*d = d.getStack() // 正在叠猫猫的队列
	...
	if l == 0 {
		...
		sendWithImageFail(handler, r[0], `猫堆中没有猫猫可以吃——`)
		return false
	}
	if t := (*d)[l-1].getTypeID(handler); t >= 小老虎 {
		// 老虎以上无法被吃
		asyncSendEmoji(handler, `😁 呲牙`)
		sendWithImageFail(handler, r[0], `不可以吃`, &t, `——`)
		return false
	}
	if t := (*d)[0].getTypeID(handler); t >= 猫猫巴士 {
		// 底座是猫猫巴士以上无法被吃
		asyncSendEmoji(handler, `✨ 闪光`)
		sendWithImageFail(handler, r[0], `不可以吃`, &t, `载的猫猫——`)
		return false
	}
	var (
		mv   = m
