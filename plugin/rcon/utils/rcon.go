...
// RCON
func Command(msgr *kitten.Messager, cp core.Path) message.ID {
	mu.RLock()
	defer mu.RUnlock()
	config, err := core.Load[rcon](cp, core.Empty)
	if err != nil {
...
// 设置 RCON
func Set(msgr *kitten.Messager, i item, cp core.Path) message.ID {
	...
	if err = core.Save(cp, config); err != nil {
