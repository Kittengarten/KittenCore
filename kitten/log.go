...
var Log *zap.SugaredLogger // 全局日志变量
...
func LogConfigInit(config Config) {
	var (
		...
		core    = zapcore.NewCore(encoder, zapcore.NewMultiWriteSyncer(zapcore.AddSync(os.Stdout), logWriteSyncer), config.Log.GetLogLevel())
		log_    = zap.New(core, zap.AddCaller(), zap.AddCallerSkip(2)) // 配置日志记录器
	)
	defer func() {
		if err := log_.Sync(); !Check(err) {
			log_.Sugar().Errorf("主函数日志刷新失败喵！\n%v", err)
		}
	}()
	Log = log_.Sugar() // 全局日志记录器
}
